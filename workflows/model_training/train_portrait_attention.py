...
RUN_NAME = os.getenv("SPTA_RUN_NAME", "portrait_attention")
...
def resolve_base_run_dir() -> Path:
    ...
    if preferred.exists():
        return preferred
    fallback = OUTPUT_ROOT / "rna_language_alignment"
    if fallback.exists():
        return fallback
    return preferred
...
def load_training_module(path: Path):
    spec = importlib.util.spec_from_file_location("portrait_attention_train_base", path)
